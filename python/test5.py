...
class WeatherAppBase:
    ...
    # 추가 멘트를 제공하는 함수 (다형성 구현 부분)
    def additional_comment(self, weather_data):
        raise NotImplementedError("이것은 추상메소드입니다.")
...
class WeatherApp(WeatherAppBase):
    def show_weather(self):  # 오버라이딩
        ...
        if weather_data and weather_data["cod"] == 200:  # 200 = 정상 상태 코드
            ...
            comment = self.additional_comment(weather_data)
    ...
    def additional_comment(self, weather_data):
        ...
        if "rain" in main_weather:
            comments.append("비가 오고 있습니다. 우산을 챙기세요!")
        if temperature > 30:
            comments.append("오늘은 매우 덥습니다. 충분한 수분을 섭취하세요!")
        if feels_like < 0:
            comments.append("체감 온도가 영하입니다. 따뜻하게 입으세요!")
        if humidity > 80:
            comments.append("습도가 높습니다. 불쾌지수가 올라갈 수 있으니 주의하세요.")
        if wind_speed > 10:
            comments.append("바람이 강합니다. 바람에 주의하세요!")
...
if __name__ == "__main__":
    ...
    # 두 번째 도시 날씨 정보 객체 생성
    app2 = WeatherApp(root, "두 번째 도시 이름을 입력하세요:", 10)
